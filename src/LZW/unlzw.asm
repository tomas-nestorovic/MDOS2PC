ZAZ_LED	.EQU	4		; pocet bajtu jednoho Zaznamu ve Slovniku dekomprese
SLOVNID	.EQU	SLO_PLN+(SLO_MAX*ZAZ_LED)	; adresa zacatku Slovniku dekomprese


	;push	bc		; prvni logicky sektor ke kompresi (zakomentovano protoze vlozeno v nadmodulu)
	;push	de		; logicky sektor k dekompresi (zakomentovano protoze vlozeno v nadmodulu)


	ld	(_SPD+1),sp	; zaloha SP
	...
	ld	hl,SLOVNID	; HL' = < bez informace > (zde pouze inicializace prazdneho Slovniku)
				; DE' = < bez informace >
...
SektorD	di
	...
	ld	d,c		; do D'=C'=0
	ld	hl,___1stD	; zmena adresy skoku instrukce Jp(...) = pro prvni symbol nelze vytvorit Zaznam ve Slovniku (protoze neexistuje predchozi symbol)
	ld	(_PrvniD+1),hl
	exx
...
SloPlnD	ld	hl,SLOVNID	; Slovnik plny - vyprazdneni
...
_Symb2D	ex	de,hl		; symbol do HL
	...
	add	hl,hl
	add	hl,hl
	;ld	a,h		; H+=High(SLO_PLN) (protoze Slovnik zarovnan na $xx00)
	;add	a,SLO_PLN>>8
	;ld	h,a
	set	6,h		; ekvivalent k vyse zakomentovanemu protoze High(SLO_PLN)=$40
	...
	.BYTE	$3e		; meni nasledujici instrukci na "neskodnou" Ld(A,...)
...
ZaznamD				; ve Slovniku vytvoreni Zaznamu [AktPrefix,AktZnak] (pouze pokud se NEjedna o prvni zapisovany symbol do Buffru - tehdy zaruceno ze pro nej existuje predchozi symbol)
	...
	exx
				...
				; zaplnen-li Slovnik, vyprazdneni (6.bit=0 = D'<High(SLO_PLN)=$40 )
	ld	h,d		; H'=L'=D'=0
	ld	l,d
	add	hl,sp
	;ld	a,d
...
_SPD	ld	sp,$0000	; obnova SP ($0000 = urceno za behu)
	...
	call	DWRITE
	call	D80Err		; test pouze chyby CRC (rutina DWRITE=$23BE nezohlednuje pripravenost mechaniky)
	pop	de
